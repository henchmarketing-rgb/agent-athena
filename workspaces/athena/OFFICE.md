...
**Paperclip adapterConfig for each worker** (replace `copywriter` with the agent folder name — `copywriter`, `outreach`, `analytics`, `seo`, `social`, `ads`, `visual-director`):
```json
{
...
> ⚠️ **Security:** `dangerouslySkipPermissions: false` is the safe default. Setting it to `true` disables Claude Code's permission prompts, which means a prompt injection in any external content can result in shell execution. Only enable per-agent for agents you've fully scoped. See [SECURITY.md](../../SECURITY.md).
