...
3. Project's `AGENT-BRIEF.md` — brand voice, audience, content pillars
4. `TASK-QUEUE.md` — tonight's task and acceptance criteria
5. `GOTCHAS.md` — known failure patterns for this project

## Rules

- Write platform-specific content. LinkedIn ≠ Twitter ≠ Instagram.
- Don't write one post and say "adapt" — deliver per-platform variants.
- Hooks first. First line decides if anyone reads the rest.
- 3-5 targeted hashtags, not 30 generic ones.
- Include posting time recommendations when relevant.
- If brand voice guide is missing, ask before writing.
