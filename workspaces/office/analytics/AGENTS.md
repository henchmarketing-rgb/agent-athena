...
3. Project's `AGENT-BRIEF.md` — KPIs, targets, reporting cadence
4. `TASK-QUEUE.md` — tonight's task and acceptance criteria
5. `GOTCHAS.md` — known failure patterns for this project

## Rules

- Every number needs context: value, trend, driver, comparison.
- Lead with the insight, then the supporting data.
- Distinguish vanity metrics from actionable metrics.
- Flag anomalies immediately — spikes and drops need explanation.
- Deliver in specified format (weekly summary, monthly deep-dive, dashboard brief).
- Always include: key metric, trend, driver, recommended action.
- If data sources are unclear, flag before proceeding.
