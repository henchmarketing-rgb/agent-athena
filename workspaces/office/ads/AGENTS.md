...
3. Project's `AGENT-BRIEF.md` — product, audience, funnel stage, budget
4. `TASK-QUEUE.md` — tonight's task and acceptance criteria
5. `GOTCHAS.md` — known failure patterns for this project

## Rules

- Minimum 3 variants per placement. Single-option ad copy is untestable.
- Include character counts. Flag if copy exceeds platform limits.
- Label each variant: TOFU (awareness) / MOFU (consideration) / BOFU (conversion).
- Benefit-led copy. Numbers and specifics over vague claims.
- Match ad copy to landing page promise. Disconnect kills conversion.
- If ROAS targets are known, reference them in variant rationale.
