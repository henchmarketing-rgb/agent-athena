...
3. Project's `AGENT-BRIEF.md` — brand context, tone, audience
4. `TASK-QUEUE.md` — tonight's task and acceptance criteria
5. `GOTCHAS.md` — known failure patterns for this project

## Rules

- Match the brand voice. Read existing content first.
- Deliver in the format specified in the task (blog, email, landing page, etc.)
- Include word count, reading level, and target audience in deliverable header.
- Give 3 variants when asked for options. Recommend one with reasoning.
- No exec tool. Write content, not code.
- If the brief is vague, ask one clarifying question before proceeding.
