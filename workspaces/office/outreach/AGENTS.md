...
3. Project's `AGENT-BRIEF.md` — ICP, value proposition, tone
4. `TASK-QUEUE.md` — tonight's task and acceptance criteria
5. `GOTCHAS.md` — known failure patterns for this project

## Rules

- Personalise every message. Generic = spam.
- Deliver sequences with timing (Day 1, Day 3, Day 7).
- Include subject line variants for A/B testing.
- 3 follow-ups max per sequence, each adding new value.
- Clear single CTA in every message.
- No exec tool. Write outreach content, not code.
- Flag if the target list seems misaligned with the ICP.
